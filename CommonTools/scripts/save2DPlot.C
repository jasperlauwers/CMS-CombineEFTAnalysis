...
const float intlumifbinv = 3000;
const int   beamcometev  = 14;
...
{
  if (parname.EqualTo("LS0") )  return "S0";
  if (parname.EqualTo("LS1") ) return "S1";
...
//   keys.push_back("obs95");
  ...
  for( size_t i=0; i<fnames.size(); i++) {
    ...
    for (size_t j = 0, n = t->GetEntries(); j < n; ++j) {
      t->GetEntry(j);
      printf ("%d\r",j);
      if( 2*deltaNLL < 0 || 2*deltaNLL > 8 ) deltaNLL = 4;
      if( !iToy){
	//	cout <<"!iToy" << endl;
		    ...
		    TLegend *legend)
{
  ...
  TCanvas *finalPlot = new TCanvas("final","limits",1500,1500);
  finalPlot->cd();
...
//   }
  ...
  finalPlot->Print(Form("%s.pdf",plotprefix.Data()));
//   finalPlot->Print(Form("%s.png",plotprefix.Data()));
  finalPlot->SaveAs(Form("%s.png",plotprefix.Data()),"png");
...
void save2DPlot()
{
  ...
  fnames.push_back("higgsCombinetest_VBS_LS0_LS1.MultiDimFit.mH120.root");
//   fnames.push_back("higgsCombinetest_VBS_LS0_LS1_obs.MultiDimFit.mH120.root");
  ...
  //
  //if (fnames[0].Contains("LS0",TString::kIgnoreCase)) {       par1 = TString("LS0");
  //  if (fnames[0].Contains("LS1",TString::kIgnoreCase))      par2 = TString("LS1");
  //  else if (fnames[0].Contains("dg1",TString::kIgnoreCase)) par2 = TString("dg1");
  //} else if  (fnames[0].Contains("LS1",TString::kIgnoreCase) &&
